...
첫 Wedge가 복잡하거나 데이터 접근 불가로 판정돼도, 사용자가 입력을 직접 제공하고 수동 결과를 만들 수 있다면 한 번은 `입력 1개 → 출력 1개`로 단순화한 뒤 전체 Product Gate에 재진입합니다. 동일 후보의 두 번째 Wedge도 실패하거나 실제 외부 데이터·공급자 의존이 남으면 HOLD합니다.
